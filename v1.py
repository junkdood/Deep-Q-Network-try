...
class GameState:
    def __init__(self):
        ...
        self.playerx = int(1)
        self.playery = int(1)
        self.goalx = int(SCREENWIDTH - 2)
        self.goaly = int(SCREENHEIGHT - 2)
        self.map = np.zeros((SCREENWIDTH, SCREENHEIGHT))
        self.finu = 0
        [rows, cols] = self.map.shape
        for i in range(rows - 1):
            for j in range(cols - 1):
                if i == self.playerx and j == self.playery:
                    self.map[i, j] = -1
    ...
    def frame_step(self, input_actions):
        terminal = False
        a = 0
        predis = (((self.playerx - self.goalx) * (self.playerx - self.goalx)) + (
                    (self.playery - self.goaly) * (self.playery - self.goaly)))
        if sum(input_actions) != 1:
            self.score = self.score - 2
            a = -2
        elif input_actions[0] == 1:
            if self.playery < SCREENHEIGHT - 1:
                self.playery = self.playery + 1
                self.score = self.score + self.map[self.playerx, self.playery]
                a = self.map[self.playerx, self.playery]
            else:
                self.score = self.score - 2
                a = -2
            ...
            if self.playerx < SCREENWIDTH - 1:
                ...
                self.score = self.score - 2
                a = -2
        dis = (((self.playerx - self.goalx) * (self.playerx - self.goalx)) + (
                    (self.playery - self.goaly) * (self.playery - self.goaly)))
        if a == -1:
            if dis > predis:
                a = -0.1
            ...
            a = 0
        #print(dis,predis,input_actions)
        ...
        for i in range(rows - 1):
            for j in range(cols - 1):
                ii = self.playerx - 3 + i
                jj = self.playery - 3 + j
                if ii < 0 or jj < 0 or ii > SCREENWIDTH - 1 or jj > SCREENHEIGHT - 1 or self.map[ii, jj] < -500:
                    submap[i, j] = 0
                elif self.map[ii, jj] > 500:
                    submap[i, j] = 255
                else:
                    # submap[i, j] = 1
                    submap[i, j] = 125
    ...
    def printt(self):
        ...
        plt.ylim(-5, 405)
        plt.pause(0.0001)
        #plt.savefig("fi/my_fig%d.png" % self.finu)
        #plt.clf()  # 清图。
        #plt.cla()  # 清坐标轴。
        #plt.close()  # 关窗口
        self.finu = self.finu + 1
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    checkpoint = tf.train.get_checkpoint_state("saved1_networks")
    ...
    while "flappy bird" != "angry bird":
        ...
        if t % 10000 == 0:
            saver.save(sess, 'saved1_networks/' + GAME + '-dqn', global_step = t)
